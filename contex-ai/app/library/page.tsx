...
export default function LibraryPage() {
  ...
  return (
    <>
      <AgentContext
        controlId="page-context-library"
        context={`
          PAGE: Library
          Has controls:
            - "click-me-button"
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <main className="flex-1 p-6 overflow-y-auto">
            ...
            <p className="text-gray-300 mt-2 mb-6">
              Easily copy and use pre-built AI-friendly React components.
            </p>
                ...
                >
                  ...
                  <div className="p-4 bg-gray-800 rounded-md">
                    {component.name === "Button" && (
                      <AgentButton
                        controlId="click-me-button"
                        onUniversalClick={() => setIsClicked(!isClicked)}
                        className={`bg-purple-700 hover:bg-purple-800 text-white px-4 py-2 rounded-md 
                          ${isClicked ? "bg-cyan-500" : "bg-purple-700"} rounded-md`}
                      >
                        Click Me!
                      </AgentButton>
                    )}
                    {component.name === "Card" && (
                      <Card className="p-6 border border-gray-700 bg-gray-900 text-white">
                        This is a card.
                      </Card>
                    )}
                    ...
                    {component.name === "Chart" && (
                      <AgentChart
                        controlId="demo-chart"
                        className="w-full h-80 lg:col-span-2"
                      />
                    )}
                    {component.name === "Toggle" && (
                      <AgentSwitch controlId="demo-toggle" className="" />
                    )}
                    {component.name === "Navbar" && (
                      <AgentNavbar controlId="demo-navbar" className="" />
                    )}
                  </div>
                  {copied === component.name && (
                    <p className="mt-2 text-sm text-green-400">Copied!</p>
                  )}
                </div>
              ))}
            </div>
